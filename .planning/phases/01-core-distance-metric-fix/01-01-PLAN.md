...
8. **find_second_nearest()** (line ~81): Change signature to accept `pixel_chroma: f32`. Pass it through to `palette.distance(color, palette.actual_oklab(i), pixel_chroma, i)`.

9. **BlueNoiseDither::dither()** (line ~130-134): Compute `pixel_chroma` from the oklab pixel:
   ```rust
   let pixel_chroma = (oklab.a * oklab.a + oklab.b * oklab.b).sqrt();
   ```
   Pass `pixel_chroma` to both `palette.find_nearest(oklab)` -- wait, `find_nearest` computes it internally. But `find_second_nearest` needs it. So compute pixel_chroma here and pass it to `find_second_nearest(oklab, palette, idx1, pixel_chroma)`.

   Actually, `find_nearest()` already computes pixel_chroma internally. The issue is `find_second_nearest()` which is a free function. Two approaches:
   - Option A: Compute pixel_chroma in the dither loop and pass to find_second_nearest
   - Option B: Make find_second_nearest compute its own pixel_chroma

   Use Option A (consistent with research recommendation -- compute once, pass everywhere):
   ```rust
   let pixel_chroma = (oklab.a * oklab.a + oklab.b * oklab.b).sqrt();
   ```
   And update the find_second_nearest call to pass it.
