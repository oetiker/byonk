...
async def test_add_device_no_already_configured_when_reconcile_races(hass):
    """Regression: onboarding must not abort 'already_configured' when the device
    becomes registered (a coordinator refresh would otherwise reconcile-create the
    subentry before the flow's own create_entry)."""
    state = {"added": False}
    dev_ccdd = {**DEV, "key": "CC:DD"}
    get_devices = AsyncMock(side_effect=lambda *a, **k: [dev_ccdd] if state["added"] else [])
    add_device = AsyncMock(
        side_effect=lambda *a, **k: state.update(added=True) or {"key": "CC:DD", "screen": "transit"}
    )
    entry = MockConfigEntry(domain=DOMAIN, unique_id=DOMAIN,
        data={CONF_ADDON_SLUG: "abcd_byonk", CONF_BASE_URL: "http://addon:3000"})
    entry.add_to_hass(hass)
    with (
        patch("custom_components.byonk.async_read_token", new=AsyncMock(return_value="tok")),
        patch("custom_components.byonk.coordinator.ByonkClient.async_get_devices", new=get_devices),
        patch("custom_components.byonk.coordinator.ByonkClient.async_get_pending", new=AsyncMock(return_value=PENDING)),
        patch("custom_components.byonk.coordinator.ByonkClient.async_get_screens", new=AsyncMock(return_value=SCREENS_NO_PARAMS)),
        patch("custom_components.byonk.coordinator.ByonkClient.async_get_config", new=AsyncMock(return_value={})),
        patch("custom_components.byonk.coordinator.ByonkClient.async_add_device", new=add_device),
    ):
        await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
        result = await hass.config_entries.subentries.async_init(
    ...
    subs = [s for s in entry.subentries.values() if s.unique_id == "CC:DD"]
    assert len(subs) == 1
